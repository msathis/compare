...
function Compare() {
    ...
    this.diff = function(i) {
        const url = 'file://' + __dirname + '/diff.html?left=' + encodeURIComponent(this.folderList.left.path + '/' + (this.comparedResults[i].left || this.comparedResults[i].right)) + '&right=' + encodeURIComponent(this.folderList.right.path + '/' + (this.comparedResults[i].right || this.comparedResults[i].left)) + '&index=' + i;
        const mergeWindow = new BrowserWindow({
          width: 800,
          height: 600,
          icon: 'assets/images/icon.icns',
          title: 'Merge - Compare Folders'
        });
        mergeWindow.loadURL(url)
    }
    ...
    this.showFileDialog = function(el) {
    ...
    this.compareFolders = function(left, right, done) {
        var leftFiles = left.files,
            rightFiles = right.files;
        var results = [],
            i = 0,
            j = 0,
            total = leftFiles.length + rightFiles.length;
    ...
    this.prepareLayout = function(bothSidesReady) {
    ...
    this.displayResults = function(files) {
        ...
        var rightHtml = '<li class="collection-header"><h4>' + rightFolderName + '</h4></li>';
        ...
        for (var i in files) {
            ...
            var $li = $('<li class="fadeIn animated collection-item ' + classString + '">' + $('#spinner_template').html() + '</li>');
    ...
    this.populateCompareStatus = function(files, i) {
        var color = '',
            i = 0;
        this.compareFile(files, i, color);
    }

    this.compareFile = function(files, i, color) {
    ...
    this.refreshCompareStatus = function(i) {
        this.isFilesSame(this.comparedResults[i], function(sameName, sameContent) {
            this.setCompareStatus(i, sameName, sameContent);
        }.bind(this));
    }

    this.isFilesSame = function(file, done) {
    ...
    this.setCompareStatus = function(i, sameName, sameContent) {
        var html = '',
            color = '';
